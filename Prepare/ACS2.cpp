...
// НОВА ФУНКЦІЯ ДЛЯ ТИПУ FLOAT
double test_float(char op) {
    long n = N_BASE;
    if (op == '/') n /= 5;

    double sum_ops = 0;
    volatile float a = 10, b = 20, c = 0; // Змінено на float
    if (op == '/' || op == '*') { a=10000; b=2; c=1; }
    ...
    for (int k = 0; k < RUNS; k++) {
        ...
        float ta = a, tb = b, tc = c; // Змінено на float
...
int main() {
    ...
    // ДОДАНІ ТЕСТИ ДЛЯ FLOAT
    v.push_back({"flt +", test_float('+')});
    v.push_back({"flt -", test_float('-')});
    v.push_back({"flt *", test_float('*')});
    v.push_back({"flt /", test_float('/')});
